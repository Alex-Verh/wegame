...
const userLanguages = computed<{ [id: number]: boolean }>(() =>
  user.languages
    ? user.languages.reduce((acc, curr) => ({ ...acc, [curr.id]: true }), {})
    : {}
);
...
const showedLanguages = computed(() => {
  if (languages.value)
    return langSearch.value
      ? languages.value.filter((language) =>
          language.title.toLowerCase().includes(langSearch.value.toLowerCase())
        )
      : languages.value;
  return [];
});
  ...
  <Popup :width="700" class="languagespop">
    <Container>
      ...
      <div class="languages_section">
        ...
        <Row class="g-1">
          <Col
            v-for="language in showedLanguages"
            :key="language.id"
            @click="toggleLanguage(language.id)"
            col="6"
          >
            <div
              :class="{ language_selected: userLanguages[language.id] }"
              class="language"
            >
              {{ language.title }}
            </div>
        ...
        >
          ...
          />
          <div @click="updateAge(userAge)" class="button_accent">
            {{ $t("enter") }}
          </div>
        </div>
        ...
        >
          <input
            @keyup.enter="updateEmail(userEmail)"
            v-model="userEmail"
            class="languages_input"
            type="email"
            name="user_email"
          ...
          />
          <div @click="updateEmail(userEmail)" class="button_accent">
            {{ $t("change") }}
          </div>
        </div>
        ...
        >
          <input
            @keyup.enter="updatePassword(userPassword)"
          ...
          />
          <div @click="updatePassword(userPassword)" class="button_accent">
            {{ $t("change") }}
          </div>
        </div>

        <button @click="logout" class="button_accent button_pop">
